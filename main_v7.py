...
def run(queue_input, queue_output, name_cam, model_cam):
    global Logging
    ...
    while True:
        if not queue_input.empty() :
            ...
            BG = detectPeoplev5.draw_box_text(BG,(0,0), (width, int(height/18)), string=text)
            queue_output.put(BG)
...
def run_process_CAM1():
    ...
    process_CAM1 = Process(name= 'process_CAM1',target=run, args=(input_cam_CAM1, output_cam_CAM1,'cam1',model_555,))
    process_CAM1.start()
...
def run_process_CAM2():
    ...
    process_CAM2 = Process(name= 'process_CAM2',target=run, args=(input_cam_CAM2, output_cam_CAM2,'cam2',model_554,))
    process_CAM2.start()
